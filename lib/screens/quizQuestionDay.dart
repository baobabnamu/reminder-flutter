...
class QuizList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    void checkIdx() {
      if (quizSet!.length == controller.quizDayIndex) {
        controller.quizDayIndex = 0;
      }
    }
    ...
    if (quizSet!.isEmpty) {
      return Text(
    ...
    } else if (!controller.quizDayCardState) {
        controller.changeQuizDayNowCardWord(quizSet![controller.quizDayIndex].word);
        return Text(
          quizSet![controller.quizDayIndex].word,
          style: TextStyle(
            color: Colors.indigo.shade900,
            fontSize: 35.0,
            fontWeight: FontWeight.w800,
        ...
        return Text(
          quizSet![controller.quizDayIndex].mean,
          style: TextStyle(
            color: Colors.indigo.shade900,
            fontSize: 35.0,
            fontWeight: FontWeight.w800,
...
class QuizQuestionDay extends StatelessWidget {
  const QuizQuestionDay({
    Key? key,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    Future<http.Response> updateQuizSpace() {
      return http.put(
        ...
        body: jsonEncode(<String, dynamic>{
          'word': controller.quizDayNowCardWord,
          'quizSpace': 3,
          'uid': controller.quizUser.value.email,
        }),
      );
